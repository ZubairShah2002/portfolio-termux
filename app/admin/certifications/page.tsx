...
const BLANK: Omit<Certification,'id'> = { name:'', issuer:'', year:'', color:'#6ee7b7', imageUrl:'', credUrl:'', order:0 }
...
export default function CertificationsPage() {
  ...
  useEffect(() => { if (!getAdminAuth()) { router.push('/admin/login'); return }; setItems(getCertifications()) }, [router])

  const openNew = () => { setEditing(null); setForm({ ...BLANK, order: items.length }); setModal(true) }
  const openEdit = (c: Certification) => { setEditing(c); setForm({ name:c.name, issuer:c.issuer, year:c.year, color:c.color, imageUrl:c.imageUrl, credUrl:c.credUrl, order:c.order }); setModal(true) }
  ...
  return (
    <div>
      <PageHeader title="Certifications" subtitle={`${items.length} certs`} action={<Btn onClick={openNew}>+ Add Cert</Btn>} />
      {items.length === 0 ? <Empty label="Certification" onAdd={openNew} /> : items.map(c => (
        <Row key={c.id} onClick={() => openEdit(c)}>
          <div style={{ width: 36, height: 36, borderRadius: 9, background: `${c.color}15`, border: `1px solid ${c.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: c.color, fontWeight: 900, fontSize: 15, flexShrink: 0 }}>✓</div>
          <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 7 }}>
            <Btn onClick={ev => { ev?.stopPropagation(); openEdit(c) }} variant="secondary" size="sm">Edit</Btn>
            <Btn onClick={ev => { ev?.stopPropagation(); del(c.id, c.name) }} variant="danger" size="sm">Delete</Btn>
          </div>
        </Row>
      ))}
      <Modal open={modal} onClose={close} title={editing ? 'Edit Certification' : 'New Certification'}>
        <form onSubmit={save}>
          <Input label="Certification Name" value={form.name} onChange={set('name')} required placeholder="AWS Solutions Architect" />
          <Input label="Issuer" value={form.issuer} onChange={set('issuer')} required placeholder="Amazon Web Services" />
