...
const BLANK: Omit<Project, 'id'> = {
  name: '', description: '', tech: [], color: '#6ee7b7',
  emoji: '🚀', githubUrl: '', liveUrl: '', featured: false, order: 0,
}
...
export default function ProjectsPage() {
  ...
  const openEdit = (p: Project) => {
    setEditing(p)
    setForm({ name: p.name, description: p.description, tech: [...p.tech], color: p.color, emoji: p.emoji, githubUrl: p.githubUrl, liveUrl: p.liveUrl, featured: p.featured, order: p.order })
    setModal(true)
  }
  ...
  return (
    <div>
      <PageHeader
        title="Projects"
        subtitle={`${items.length} project${items.length !== 1 ? 's' : ''} in your portfolio`}
        action={<Btn onClick={openNew}>+ Add Project</Btn>}
      />

      {items.length === 0 ? (
        <Empty label="Project" onAdd={openNew} />
      ) : (
        items.map(p => (
          <Row key={p.id} onClick={() => openEdit(p)}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {p.featured && (
                  <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 5, background: 'rgba(110,231,183,0.12)', color: '#6ee7b7', fontWeight: 700 }}>FEATURED</span>
                )}
              </div>
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {p.description}
              </div>
              <div style={{ display: 'flex', gap: 5, marginTop: 5, flexWrap: 'wrap' }}>
                {p.tech.slice(0, 4).map(t => (
                  <span key={t} style={{ fontSize: 10, padding: '2px 6px', borderRadius: 5, background: `${p.color}12`, color: p.color, border: `1px solid ${p.color}20` }}>{t}</span>
                ))}
              </div>
            </div>
      ...
      <Modal open={modal} onClose={close} title={editing ? 'Edit Project' : 'New Project'}>
        <form onSubmit={save}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px', gap: 10, alignItems: 'end' }}>
            <Input label="Project Name" value={form.name} onChange={set('name')} required placeholder="My Awesome Project" />
          ...
          <TagInput label="Tech Stack" value={form.tech} onChange={set('tech')} placeholder="Add technology, press Enter" />
          ...
          <Toggle label="Featured project" value={form.featured} onChange={v => setForm(f => ({ ...f, featured: v }))} />
          <div style={{ display: 'flex', gap: 9, justifyContent: 'flex-end', marginTop: 6 }}>
            <Btn onClick={close} variant="ghost">Cancel</Btn>
            <Btn type="submit">{editing ? 'Update Project' : 'Create Project'}</Btn>
          </div>
        </form>
      </Modal>
